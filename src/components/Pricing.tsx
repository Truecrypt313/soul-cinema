...
import { track, setInterestPackage } from '@/lib/analytics'
            ...
            return (
              <FadeUp key={p.id} delay={i * 0.07} className="h-full">
                <div className={`relative h-full flex flex-col rounded-2xl p-8 gentle-animation ${
                  ...
                  <a href="#contact"
                    onClick={() => { setInterestPackage(p.name); track({ event_name: 'pricing_cta_click', cta_id: 'pricing_card', metadata: { package_slug: p.name, package_tier: p.highlighted ? 'highlighted' : 'standard' } }) }}
